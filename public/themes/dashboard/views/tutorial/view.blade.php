...
<div class="row">
    ...
    echo Form::text('id',$id,array('disabled'=>'','class'=>'disabled pull-right','style'=>'margin:10px;','readonly'));

    echo Form::label('title','Title',array('class'=>'pull-left','style'=>'clear:left;margin:15px;'));

    echo Form::text("title",$tutorial->name,array('placeholder'=>'Title of the Tutorial','class'=>'pull-right','style'=>'clear:right;margin:10px;','readonly'));

    echo Form::label('description','Description',array('class'=>'pull-left','style'=>'clear:left;margin:15px;'));

    echo Form::text("description",$tutorial->description,array('placeholder'=>'Describe the Tutorial Here','class'=>'pull-right','style'=>'clear:right;margin:10px;','readonly'));
    ...
    echo Form::select('subject',$subjectlist,$tutorial->subjectid,array('class'=>'form-control pull-right','style'=>'clear:right;margin:10px;','readonly'));
    ?></div>
...
<div class="clearfix"></div>
    ...
    echo '<div class="make-switch pull-right"><input  type="checkbox" name="published" id="published" '.$checked.' readonly></div>';
    ...
    echo '<div class="make-switch pull-right"><input value="on" type="checkbox" name="examstruth" id="examstruth" '.$checked.' readonly></div>';
    echo Form::label('exams','Exams to Use',array('class'=>'pull-left','style'=>'clear:left;margin:15px;'));
    echo Form::select('exams',$exams,$possibleexamid,array('class'=>'pull-right','style'=>'clear:right;margin:5px;','readonly'));
    echo "</fieldset>";
    if(Sentry::getUser()->id == $tutorial->createdby){
        echo '<a class="btn btn-success" href="/tutorial/edit/'.$id.'" > Open Editable</a>&nbsp;';
    }
